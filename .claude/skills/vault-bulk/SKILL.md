...
- A Confluence space `PROD` on `redacted.atlassian.net` -> `llm-wiki-bulk-redacted-prod`
...
sources:
  - id: confluence-prod
    type: confluence
    location: https://redacted.atlassian.net/wiki/spaces/PROD
    connector-skill: redacted:confluence    # or null if local-folder/git
...
- `redacted:sharepoint`, `redacted:confluence` (work plugin)
...
>
> 1. If your work plugin has one (e.g. `redacted:confluence`), make sure it's enabled in this session.
