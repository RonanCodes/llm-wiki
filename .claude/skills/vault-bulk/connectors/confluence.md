...
2. **Work plugin connector**: `redacted:confluence`, `<employer>:confluence`, or any `*:confluence` skill provided by a loaded plugin.
...
> Options:
>
> 1. If your work plugin provides one (you mentioned `redacted` skills on your work machine that touch Confluence), enable that plugin in this session.
...
The split keeps Confluence auth out of this repo entirely. Personal-plugin and work-plugin connectors can both register under the `*:confluence` namespace, so you can have `ro:confluence` for personal stuff and `redacted:confluence` for work-tenant access without overlap.
