...
Four archetypes emerge in practice. Pick one when scaffolding a new vault. It shapes the `CLAUDE.md` body, the domain-tag pattern, and how other vaults link into this one. If unsure, ask the user which archetype fits.

**A note on Bulk vaults:** Bulk is the 4th archetype, but `/vault-create` does not scaffold it directly. Bulk vaults are managed by `/vault-bulk` because they have additional structure (`bulk-source.yaml` manifest, refresh-aware page frontmatter, `## Notes` divider contract). If the user asks for a vault that matches the Bulk profile (mirroring SharePoint, Confluence, an inherited doc folder, or a git wiki), redirect them to `/vault-bulk create <slug> --source-type <type>` instead.
...
Examples: `llm-wiki-bulk-acme-engineering` (mirror of a SharePoint site), `llm-wiki-bulk-redacted-prod` (mirror of a Confluence space), `llm-wiki-bulk-hr-archive` (mirror of a local folder of inherited docs), `llm-wiki-bulk-org-handbook` (mirror of a git repo).
...
- Populated and refreshed via `/vault-bulk` only; not hand-curated.
- Has additional vault-root files compared to other archetypes: `bulk-source.yaml` manifest, `raw/<source-id>/` mirror per source.
- Every page in `wiki/sources/` carries refresh-aware frontmatter (`imported-at`, `source-modified-at`, `last-refreshed-at`, `source-hash`, `bulk-source-id`) and ends with a `## Notes` divider that survives refresh.
- Lint rules are relaxed: orphan source-notes are expected, empty `related:` fields are normal. `/lint` reads `Archetype: Bulk` from CLAUDE.md and downgrades the matching rule classes.
- `/promote` between two Bulk vaults is forbidden. To graduate a single insight, `/promote --to <hub-vault>` converts the page to a concept-note (drops bulk frontmatter, adds `derived-from:` ref).
- Signs it's a bulk vault: hundreds-of-pages volume, no plan to curate, source-of-truth lives upstream, user explicitly wants a queryable mirror with refresh.

**CLAUDE.md flavour:** generated by `/vault-bulk create`, not this skill. Records the archetype as `Bulk`, the source type and location, and the lint-rule relaxations. See `.claude/skills/vault-bulk/SKILL.md` § "Subcommand: create" for the template.

**Cross-archetype rules:**
- Hubs and Spokes MAY cross-vault-link into a Bulk vault's source-notes when a specific upstream page is worth referencing, but Bulk vaults do NOT cross-vault-link OUT - they are leaves in the graph.
- Bulk vault refreshes commit and (optionally) push automatically; this is the only archetype where refresh is destructive-by-design above the `## Notes` divider.
